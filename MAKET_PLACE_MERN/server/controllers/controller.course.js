...
import Completed from "../models/model.completed";
...
export const uploadImage = async (req, res) => {
  //   console.log("Uploaded image => ",req.body);
  try {
    const { image } = req.body;
    if (!image) return res.status(400).send("No image");

    // prepare the image to upload s3
    ...
    //image params
    const params = {
      Bucket: "marketplaceudemy-bucket",
      Key: `${nanoid()}.${type}`,
      Body: base64Data,
      ACL: "public-read",
...
export const removeVideo = async (req, res) => {
  try {
    ...
    console.log("remove video success ===>", req.body);
    // return;
...
export const paidEnrollment = async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // redirect url sfter successful payment
