...
export const makeInstructor = async (req, res) => {
  try {
    // 1. find user from db
    console.log("its logged in ", req.auth._id);
    const user = await User.findOne({ _id: req.auth._id }).exec();
    console.log("founf user from db", user);
    ...
    // console.log("redirect url", process.env.STRIPE_REDIRECT_URL);
...
export const getAccountStatus = async (req, res) => {
  try {
    ...
    if (account.charges_enabled) {
      // user logout with charges_enabled is true
      return res.status(401).send("Unauthorized");
    } else {
      console.log("user callback running with charges_enabled is false");
